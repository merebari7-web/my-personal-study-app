...
| ⚡ Pro Boost | **v28.0** — productivity + motion layer, another lazy boot-safe module (`quiz/boost.js`, idle-loaded after pro). **⚡ Math Sprint** — 60-second mental-arithmetic arcade pitched for JAMB/WAEC speed: 3 levels, on-screen numpad + keyboard, combo streak with rising chime pitch, live timer bar, best-per-level, XP + coins rewards, play-again loop. **🌧️ Soundscapes** — generative WebAudio ambient study audio (rain · ocean · brown noise) that plays offline, with volume slider and saved preset — pairs perfectly with Zen Focus. **🧊 3D tilt + light glare** on interactive tiles/cards (desktop pointers only, reduced-motion aware, resets on leave). **💫 rotating gold aura ring** on the Quiz and Plan cards. Both new tools are in the Ctrl+K palette. Verified: v28 suite 12/12 (real-Chrome 375/1280: sprint runs to a result view and persists best, soundscape audio starts and stops, tilt transform applies and resets, conic ring renders, zero page errors, no overflow), v20/v23/v24/v25/v26/v27 regressions green, 12-viewport audit 0 issues, boot wire 264,776 B ≤ 266,240 B (unchanged — lazy module only), bank.js untouched, SW `-v28`. |
