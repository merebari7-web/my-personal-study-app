...
| 📖 NERDC Lesson Notes | **v25.0** — a **NERDC-curriculum content platform** for Nigerian primary and secondary schools, shipped inside the app: **Primary (Basic 1–6)** with the full scheme map for **13 subjects** × 3 terms × 10 weeks (Mathematics, English Studies, Basic Science & Technology, Social Studies, Civic Education, CRS, Agricultural Science, Home Economics, Cultural & Creative Arts, Computer Studies, PHE, Verbal & Quantitative Reasoning) and a **Secondary** lane that links to the existing 19-subject syllabus/T teaching suite. **Basic 1 Mathematics carries 30 hand-authored full lesson notes** — all three terms — each with the 8-part structure the examiners want (learning objectives, previous knowledge, instructional materials, lesson content, worked examples, class activities, evaluation questions, assignment), reachable through a new **📖 Notes** launcher: searchable browser (level → class → subject → term → week), a **Scheme-of-Work generator** with printable output, **My Space** role dashboards (Student progress + bookmarks, Teacher scheme + Author mode, Parent overview), on-device authoring (PIN: NERDC2026) with JSON export, and print/PDF export for any note. Docs: `docs/ARCHITECTURE.md` (full architecture + data model) and `docs/schema.sql` (Supabase/Postgres schema with RLS + approval workflow) for the hosted upgrade path. Lazy data+UI modules (`quiz/notes_data.js`, `quiz/notes_app.js`) keep boot at **264,776 B ≤ 266,240 B**, `bank.js` untouched, SW `-v25`. Verified: v25 suite 11/11 (30-note × 8-part data audit, jsdom e2e: browse → note → bookmark → done → author save → search; real-Chrome 375/1280 e2e, no overflow/errors), v20/v23/v24 regressions green, 12-viewport audit 0 issues. |
