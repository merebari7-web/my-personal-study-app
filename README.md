...
**19 subjects of SS1–SS3 lesson notes** · the complete **684-topic SS1–SS3 syllabus map** (term by term) · 3,900 verified questions · **v16 Lumina UI refresh** (gold-on-ivory buttons, cards, AI Coach medallions) with full explanations · JAMB/UTME simulation with on-screen calculator · essay/theory practice · study analytics · games + a video studio · tiny files (index.html + bank.js + on-demand labs.js / edu.js / arcade.js) — no install, no internet, no dependencies.
...
| 📊 Study Analytics | **v14.0**
| 📚 Complete Curriculum Library | **v15.0** — *"all learning materials for SS1–SS3 students"*: a full **684-topic syllabus index** for all 19 WAEC/NECO subjects laid out **term by term (1st / 2nd / 3rd) across SS1–SS3** in the 🧭 Syllabus Map — plus a hand-authored **lesson-note & question library** (19 subjects × 6 core topics with real key facts, topic keywords and **228 authored questions**, 2+ per topic with options, answers and explanations) that now leads every Lesson Note's **worked examples and class evaluation**. The 🗓 Scheme of Work becomes a genuine **12-week-per-term plan drawn from the real syllabus**, and every real topic on the map pairs with a note, practice questions and a scheme slot. The whole library ships as lazy, service-worker-cached siblings (`quiz/_syllabus_data.js` + `quiz/_curr_data.js`) that load on first use of the Teaching Suite and fall back to the keyword notes gracefully. Verified: v15 suite 12/12 + real-Chrome E2E (684 pills, 19 term-wise cards, curated questions in evaluations, 36-week scheme), v14 suite 47/47, v9 + bughunt + 12-viewport responsive audit all green, bank untouched & hash-verified, boot wire 261.6 KiB ≤ 266,240 B gate, SW cache key `-v15` |
 — the Arcade now records **every answer per subject**: a **Study Stats** dashboard (Accuracy, questions answered, arcade XP, coins, games played, 7-day XP strip) ranks your subjects with bars and puts one-tap **Rapid-drill buttons** on your three weakest subjects, so revision is targeted; **UTME quick subject packs** (🔬 Science · 🧬 Biological · 💼 Commercial · 🏛 Arts) fill the three elective subjects in one tap with checkmarked picks. Verified: v14 suite 47/47, responsive audit 0 issues across 12 viewports incl. the stats screens, bank untouched, wire gate holds, SW cache key `-v14` |
| ✨ Lumina UI refresh | **v16.0** — a gold-on-ivory design refinement across the whole app: **buttons** (rebound micro-interactions, press-down & focus-visible rings, animated **gold sheen** sweep on primary actions), **cards** (lift-on-hover with a gold hairline, layered shadows), a **gold gradient scrollbar** and soft selection tint, an **animated shimmer headline** in the hero · **🤖 AI Coach** premium suggestion cards (gold **medallion icons** with per-idea colour hues, **★ TOP PICK** ribbon, staggered entrance, gradient buttons) plus a **responsive fix** — the suggestion row no longer clips on phones ≤640px · **🎮 Study Arcade** gold-ringed glass panels, **gradient icon medallions** on every game tile, hover lift + radial gold glow, pill best-score badges, gold active chips, animated screen transitions · **🎓 Teaching Suite** gold accent header, hover-lift lesson cards, gold-tint pills, hover rows in marking tables, gold-edged notes and question blocks. Verified: v16 suite 11/11 (incl. real-Chrome E2E), v14/v15 suites green, v9 + bughunt + **responsive audit 0 issues across 12 viewports**, bank untouched & hash-verified, boot wire 263.3 KiB ≤ 266,240 B gate, SW cache key `-v16` |

| ✨ Lumina II refresh | **v17.0** — a second, deeper polish pass: **buttons/controls** — disabled-state styling, gold focus-visible on every input & button, and **touch-device hover suppression** (no more sticky hover on phones) · **🎮 Study Arcade** — gold header chrome, **animated floating medallions**, answer feedback (**correct pops green, wrong shakes red**), gold HUD pills, glass calculator display, gold UTME answer-grid cells & KPI cards, hover-lift flash/pair/video cards · **🔬 Exam-Room Labs** — gold header, glowing trophy, rotating close button, gold focus ring, correct/wrong answer states, hover-lift flash cards · **🎓 Teaching Suite** — gold-tag badges, hover-lift options and week rows · **🤖 AI Coach responsiveness** — hardened for small screens: `minmax(min(100%,…),1fr)` grid, `overflow-wrap` on every text node, 1-column stack under 400px, row buttons at 320–414px all verified overflow-free in real Chrome. Verified: v17 suite 11/11, v16/v15/v14/v9 suites green, bughunt 0 fails, **responsive audit 0 issues across 12 viewports**, bank untouched & hash-verified, boot wire 263.7 KiB ≤ 266,240 B gate, SW cache key `-v17` |

| 💎 Aurum first-class upgrade | **v18.0** — deep graphics pass: **home hero** — frosted-glass stat plates over the aurora with **gold-gradient numerals** that subtly breathe, aurum glow ring behind the 3D orbs (dark-theme matched); **🎮 Study Arcade** — ambient gold mist in the modal, **liquid-gold shimmer sweeping** through every progress bar/XP week bar, gold pill chips with inner highlight, **gold-gradient KPI figures**, corner-glow question plates, flash-card plates · **🔬 Exam-Room Labs** — **breathing gold focus ring**, floating trophy (motion-gated), gold subject & question-number chips, correct/close/wrong score pills · **🎓 Teaching Suite** — **gold circular question-number chips**, refined gold-bordered class/subject selects, gold mist on the modal, gold-tint mark-scheme tables · **shell** — **premium gold-edged night-glass toast**, gold keycap chips for keyboard hints, launcher pulse ring on hover. All animations gated behind `prefers-reduced-motion`/`.rmotion`, all heavy graphics live in the lazy modules so the boot wire stays under the gate. Verified: v18 suite 12/12 (incl. real-Chrome computed-style checks: gradient numerals, `auFlow` shimmer, gold chips, breathing ring), all v14–v17 suites + v9 + bughunt green, responsive audit 0 issues / 12 viewports, bank untouched, boot wire 264.1 KiB ≤ 266,240 B, SW cache key `-v18` |

| 📱 All-device responsive | **v19.0** — a responsiveness pass verified in real Chrome all the way down to **280px** (Galaxy Fold-class): **280px fold** — the plan gauge re-stacked as a flex column (it was a grid whose implicit columns overflowed the card), class tabs become a swipe scroll-strip on ≤479px so the SS1/SS2/SS3 caps never clip, hero orbs/rings scaled for the fold, extra 319px-and-under padding/type/stat-grid tightening · **short & landscape screens** — hero squeezes under 520px height, and the Arcade/Labs/Edu overlays switch to `100dvh` + sticky headers so toolbars never hide behind mobile browser chrome · **iOS zoom-on-focus fix** — every input/select/textarea renders at 16px on coarse pointers, killing the auto-zoom jump when tapping a field · **safe-area insets** — bottom dock, Arcade toolbar and all modals clear the home-indicator/notch cutouts on the three app surfaces · **ultra-wide monitors** — content re-centred to 1060px on 1440px+ screens · balanced headline wrapping on narrow phones. Verified: v19 suite 14/14 (real-Chrome 280px + 667px-landscape assertions: zero horizontal overflow, gauge card-fit, tab integrity, arcade overlay), all v14–v18 suites + v9 + bughunt green, 8-viewport probe + 12-viewport audit **0 issues**, bank untouched & hash-verified, boot wire 264.4 KiB ≤ 266,240 B gate, SW cache key `-v19` |

| 🖥️ AI Coach desktop layout | **v20.0** — the **🤖 AI Coach · Suggested for you** panel is now truly **desktop-responsive**: on PC the suggestion grid was auto-fitting ~3 narrow columns (240px minimum) that crammed every card — the text column collapsed to ~44px beside the action button and notes became 280px-tall walls of text. From **1024px up** the panel now uses a generous **2-column grid** (~441–481px cards), the action buttons are **vertically centred** and never squeezed, and the whole section reads like a proper dashboard. Verified: v20 suite 11/11 with real-Chrome assertions at 1280px and 1920px (2 columns, ≥150px text column, zero clipped buttons, zero horizontal overflow), all v14–v19 suites green, 8-viewport probe + 12-viewport audit 0 issues, bank untouched, boot wire 264.4 KiB ≤ 266,240 B gate, SW cache key `-v20` |

| 🧹 Mater Notes removed | **v23.0** — the **📚 Mater Notes** feature (the floating launcher, the lazy `quiz/mater_data.js` module and the `mater-notes/` PDF archive) has been **removed from the site** at the owner's request: the launcher, overlay and all 60 lesson-note PDFs are deleted from the repository and the service-worker cache key is bumped to `-v23` so every client drops the cached artifacts. Verified: v23 suite 8/8 (no Mater references anywhere in the shipped app, real-Chrome zero mater requests, boot wire 264,432 B ≤ 266,240 B gate), v20 regression green, 12-viewport audit 0 issues, bank.js untouched `cec0489e203fd4c6`. |
| ✨ Aurum Design System | **v26.0** — a full UI & graphics polish across **every** surface, still shipped as one lazy boot-safe module (`quiz/polish.js` — the app shell did not change a byte). Hero: shimmer headline, twinkling particle field, breathing halo, and a **new live today-chip** (date · current Nigerian school term · 🔥 streak). Stats: numbered gold icons + inner frames + rise-in. Nav/dock: glass, gold rims, rounded buttons. Cards/tiles: gold-edged hover lifts across the 3D lab, study hall, arcade and library rows. Quiz: option hover/press feedback, correct-answer glow, rounded qnav. Overlays: **backdrop blur** on all 14 modals. Toast: gold pill frame. Focus rings for keyboard users. **New features:** smooth **back-to-top** button and **scroll-reveal stagger** for cards (IntersectionObserver; honours reduced-motion, prints clean). Verified: v26 suite 9/9 (real-Chrome 375/1280: chip renders today's term, stat icons present, back-top appears on scroll, reveal fires, zero errors, no overflow), v20/v23/v24/v25 regressions green, 12-viewport audit 0 issues, boot wire 264,776 B ≤ 266,240 B (unchanged), bank.js untouched, SW `-v26`. |
| ⌨️ Pro Tools | **v27.0** — a productivity layer, one more lazy boot-safe module (`quiz/pro.js` loaded by polish's idle hook; the shell did not change a byte). **Ctrl+K / Cmd+K command palette** — fuzzy search across everything (papers, notes, tools, actions) with Enter-to-run, Esc to close, group labels. **Exam countdown strip** — WAEC (Fri 10 Apr 2027), NECO (Mon 17 May 2027), JAMB (Mon 22 Mar 2027) with live day counts + auto-hide after the exam. **🧘 Zen Focus timer** — Pomodoro presets (15/25/45 min), breathing ring, chime + Notification on completion, per-session log, sessions feed the streak. **Study heatmap** — GitHub-style 14-week grid from real attempt/zen history with intensity levels. **Personal touches** — time-of-day greeting, daily study tip. **Micro-interactions** — button ripple, tile-icon pop, animated section underlines. Verified: v27 suite 12/12 (real-Chrome 375/1280: palette opens/filters/runs, zen ticks 25:00, exam chips show day counts, ripple fires, zero errors, no overflow), v20/v23/v24/v25/v26 regressions green, 12-viewport audit 0 issues, boot wire 264,776 B ≤ 266,240 B (unchanged — lazy module only), bank.js untouched, SW `-v27`. |
| 🏆 Apex HQ | **v28.0** — another lazy boot-safe module (`quiz/aura.js`, idle-loaded by pro; the shell did not change a byte). **🏆 Trophy Room** — a showcase gallery for all 13 badges: unlocked cards glow gold with the unlock date, locked cards show live progress bars (mirrors of the app's own rules — e.g. 5/7 days toward "Week of fire", 28/200 toward "Centurion") and an overall unlock % bar. **🗓 Exam Sprint Plan** — a day-by-day revision plan generated from your target exam (or the nearest of WAEC/NECO/JAMB) plus your weakest/most-attempted subjects: checkable session rows with per-day focus subject + rotating activity (Rapid Fire, mistake drill, Zen…), a Start button that jumps straight into the right mode, Regenerate for a fresh plan, and clean Print/PDF output. A **🏆 Apex** pill joins the home dock (with a gold dot when open) and both screens are reachable from Ctrl+K. **v28 polish layer in polish.js** — section headers draw an animated gold underline, hero stat numerals count up on load, a soft cursor spotlight rides the stat/notes cards on desktop, the hero glow shifts hue with the time of day, the light theme gets a warm remaster, and **the permanently-broken home dock is repaired** (it never showed and its buttons never navigated — verified live: `AI.bound` stayed false; polish now binds an idempotent show-on-scroll + smooth-scroll controller). Verified: v28 suite 12/12 (real-Chrome 375/1280: dock shows + navigates + all six pills, trophy cards render, plan generates/toggles/persists, palette opens Apex, Esc closes, zero errors, no overflow), v20/v23/v24/v25/v26/v27 regressions green, 12-viewport audit 0 issues, boot wire 264,776 B ≤ 266,240 B (unchanged), bank.js untouched, SW `-v28`. |
| ⚡ Pro Boost | **v29.0** — a second lazy boot-safe module (`quiz/boost.js`, idle-loaded by polish; merged on top of Apex HQ v28 so both releases ship together). **⚡ Math Sprint** — 60-second mental-arithmetic arcade pitched for JAMB/WAEC speed: 3 levels, on-screen numpad + keyboard, combo streak with rising chime pitch, live timer bar, best-per-level, XP + coins rewards, play-again loop. **🌧️ Soundscapes** — generative WebAudio ambient study audio (rain · ocean · brown noise) that plays offline, with volume slider and saved preset — pairs perfectly with Zen Focus. **🧊 3D tilt + light glare** on interactive tiles/cards (desktop pointers only, reduced-motion aware, resets on leave). **💫 rotating gold aura ring** on the Quiz and Plan cards. Both tools join the Ctrl+K palette via pro's palette-hook API. Verified: v29 suite 12/12 (real-Chrome 375/1280: sprint runs to a result view and persists best, soundscape audio starts/stops, tilt applies + resets, conic ring renders, zero page errors, no overflow; three real bugs found & fixed by the suite — detached-subtree getElementById, scape-close arg swallowing, boot-time-only tilt guard), Apex HQ v28 suite green, v20–v27 regressions green, 12-viewport audit 0 issues, boot wire 264,776 B ≤ 266,240 B (unchanged — lazy module only), bank.js untouched, SW `-v29`. |
| 🧪 Scholar Toolkit | **v30.0** — a learning-content + motion layer, a fourth lazy boot-safe module (`quiz/toolkit.js`, idle-loaded by polish; shell untouched). **🧪 Periodic Table Explorer** — all 118 elements, category-coloured grid (alkali → noble gas, f-blocks), live search by name/symbol/atomic number, tap-to-open detail cards (mass, group, period, category) and a **🕵️ Element Detective** quiz (8 rounds, 3 question styles, XP + coins rewards). **📐 Formula Vault** — 124 WAEC/NECO/JAMB formulas across Maths (algebra, sequences, trig, mensuration, statistics), Physics (mechanics, electricity, waves, thermal) and Chemistry (mole concept, acids & bases, electrochemistry, organic), with subject tabs, cross-subject search, ★ bookmarks and one-tap copy. **Motion & graphics:** gold **scroll-progress bar** across the whole app, **entrance animation for every modal** (scale+fade on the shared `.overlay`), and a **quiz-option reveal stagger** — all reduced-motion-aware. Both tools joined the Ctrl+K palette via pro's palette-hook API. Verified: v30 suite 12/12 (real-Chrome 375/1280: 118 cells, search → detail → quiz round advance, vault search/bookmark, progress bar fills on scroll, overlay animation applied, zero errors, no overflow; three real app behaviours confirmed — smooth-scroll stubs and shared localStorage caught test-side), v20–v29 regressions green (SW assertions made version-agnostic so they never break future releases), 12-viewport audit 0 issues, boot wire 264,776 B ≤ 266,240 B (unchanged — lazy module only), bank.js untouched, SW `-v30`. |
| 🧠 Study Studio | **v31.0** — a creative + customization layer, a fifth lazy boot-safe module (`quiz/studio.js`, idle-loaded by polish; shell untouched). **🧠 Mind Map Studio** — an SVG canvas for any topic: drag nodes, tap to edit, 🔗 connect ideas with arrows, six node colours, a starter template, auto-save to the device, and one-tap **PNG/SVG export** for printing and sharing. **📏 Unit & Rate Converter** — 10 categories (temperature with its special C/F/K formulas, length, mass, volume, speed, time, pressure, energy, data, area) with swap and live results. **🎨 Theme Studio** — five accent presets (Aurum, Sapphire, Emerald, Rose, Violet) that re-colour *every* gold surface — buttons, progress bars, chips, rings, scrollbar, heatmap, palette — via CSS variables, saved automatically. **🌅 Daily Spark** — a fresh Word of the Day + Quote of the Day card in the hero, rotating daily. **☄️ Graphics** — gold comet streaks drifting across the hero (reduced-motion aware). All three tools join the Ctrl+K palette. Verified: v31 suite 12/12 (real-Chrome 375/1280: mind-map template → add → connect (dedupe proven) → drag → rename → export → persist, converter C→F 212 & km→m 1000, accent applies + persists + resets, comets ×3, spark card, zero errors, no overflow; one real bug caught & fixed — the converter's result text was destroying its own unit label on first render), v20–v30 regressions green, 12-viewport audit 0 issues, boot wire 264,776 B ≤ 266,240 B (unchanged — lazy module only), bank.js untouched, SW `-v31`. |
| ✨ Aurum Gloss | **v32.0** — a pure UI & graphics pass, all inside the existing lazy `quiz/polish.js` (no new modules, shell untouched). **Toast** — gold countdown bar that drains over the toast's 3.2 s life + spring pop-in (rebuilds itself through the app's content-wipe). **Hero badge** — a slow rotating aurum aurora ring now orbits the badge. **Glass sheen** — every card, modal, tool box, chip and tile gets a white top-glass gradient + gold corner glow, so the whole app reads as one material. **Forms** — every input, select and textarea glows gold on focus and on hover (skipping checkboxes/radios/sliders). **Motion** — XP progress bar breathes with a gold glow pulse, stat numerals gently bob after their rise-in, and heatmap cells pop on hover with a gold rim. All effects gated behind `prefers-reduced-motion`, print-safe, and scoped under `html.polished`. Verified: v32 suite 12/12 (real-Chrome 375/1280 computed-style assertions: card sheen gradient, badge conic ring spinning, input focus glow, XP glow, stat bob, toast bar rebuilds after two consecutive toasts, zero errors, no overflow), v20–v31 regressions all green, 12-viewport audit 0 issues, boot wire 264,776 B ≤ 266,240 B (unchanged), bank.js untouched, SW `-v32`. |
| 🌐 Holo 3D Lab | **v33.0** — interactive 3D, as a lazy boot-safe module (`quiz/holo.js`, idle-loaded by polish; shell untouched). **🧬 3D Molecule Viewer** — eight classic molecules (water, CO₂, ammonia, methane, O₂, N₂, H₂, benzene) drawn live: shaded CPK-coloured atoms, single/double/triple bonds (benzene gets a delocalised ring), drag-to-orbit, scroll-to-zoom, auto-rotate, labels. **📈 3D Graph Studio** — six interactive z = f(x,y) surfaces (rolling waves, saddle point, golden bowl, cone, ripple rings, sky dome) with depth-shaded facets and XYZ axes, so graph sense is built visually. **🪐 Orbital Solar System** — all eight planets orbiting a glowing sun with true relative periods (Mercury 3.1 → Neptune 60), Saturn's ring, speed slider and label toggle. Everything is pure canvas-2D (no WebGL, works offline), honours prefers-reduced-motion, and you can drag every scene even on a touchscreen. All three scenes are in the Ctrl+K palette. Verified: v33 suite 12/12 — real-Chrome 375/1280 with a **pixel-level "canvas actually painted" assertion**, which caught two real rendering bugs before ship: the orbit camera crashed on a missing `zoom` property (blank screen), and orbit geometry didn't fit small screens; both fixed. v20–v32 regressions green, 12-viewport audit 0 issues, boot wire 264,776 B ≤ 266,240 B (unchanged — lazy module only), bank.js untouched, SW `-v33`. |
| 🚫 No auto-scroll | **v38.0** — the page **never scrolls up on its own again**: the last automatic scroll-to-top (the setup-step card switch) is removed from `setCard`, so class → subject → length → quiz flows, answering, and palette jumps all hold your exact scroll position. A full audit proved it is the only one: `index.html` now contains exactly 2 `scrollTo` calls, both belonging to the home-dock navigation the learner taps deliberately. Deliberate navigation still works — Back-to-top button, dock Home/Coach/Practice/Lab, palette “Back to top” — and focus still reveals newly appeared actions below you (e.g. the Next button), which never yanks upward. Verified: v38 suite 7/7 (source: 0 top-scrolls; jsdom: all 6 cards switch with 0 scroll calls; real-Chrome 375/1280: every setup click holds scroll within 100px, answer + palette jump never move up, back-top + dock still reach the top, zero errors, no overflow), v14 + v20–v37 regressions green (v35 scroll test updated to the new 0-scroll invariant), 12-viewport audit 0 issues, boot wire 264,736 B ≤ 266,240 B (smaller — code removed), bank.js untouched, SW `-v38`. No screenshots — nothing visual changed. |
| 🎯 Zero-jump + bugfix sweep | **v39.0** — the page now **never moves on its own, up or down**: every `focus()` in the app (7 in `index.html`, palette search, notes search) passes `preventScroll`, killing the last uninvited viewport jump (question renders used to teleport the page down to the Next/Submit button). A full real-Chrome bug-hunt smoke tour (`quiz/_bughunt39.js`: full paper, review, library, HQ, labs, notes, Teaching Suite, all 10 arcade routes, holo/calc/AI/atlas/reels, 46 palette actions, toggles, corrupt-storage boot) then found and fixed **four genuine bug groups**: (1) **quiz-locked integrity failure (critical, v35+)** — the curriculum expansion pushed subjects into `QUIZ_RAW.subj` (13→27), which tripped the `===13` tamper guard on every re-boot, so ~30s after page load **every paper refused to start** (`⛔ Content integrity check failed — quiz locked`); fixed with tolerant `>=13` guards (`index.html`, `labs.js`, `edu.js`), a boot success path that resets retries, clears stale errors and re-injects curriculum questions after a bank rebuild, plus a `__curicReinject` hook in `quiz/curriculum.js`; (2) **re-boot clobbering live work** — the same bank event re-ran `boot()`, which yanked users out of a live paper or back a setup step; `boot()` now returns early during a live paper and only positions the setup card on first boot, and mock-integrity listeners are registered once; (3) **empty-quiz crashes** — answering or advancing with no live paper threw `TypeError` toasts and rendered `NaN%` / `+NaN merits`; `pick`/`nextQ`/`showResults` are now guarded and merits are NaN-proof; (4) **palette Papers rows showed `undefined · undefined`** — they read a wrong `CLASSES` shape; they now list every real `SS1/SS2/SS3 · subject` paper. Verified: v39 suite 21/21 (source: 0 bare `focus()`, guards present; real-Chrome 375/1280: focus/scroll tripwire with strict ±8px zero-jump, bank stays unlocked across re-boots with all 4,167 questions, mid-quiz/mid-setup events preserve state, empty-quiz safe, palette names real), v38 suite tightened to strict zero-jump and green, v14 + v20–v38 regressions green (v36 scroll check made settle-robust), 12-viewport audit 0 issues, boot wire 264,917 B ≤ 266,240 B, bank.js untouched, SW `-v39`. |
| 🏔️ 3D scrollable website | **v40.0** — the site is now a **3D scrollable experience**: a lazy scroll-driven depth journey (`scroll3d` in `quiz/polish.js`, exposed as `window.__s3d`) flies the 17 hero stage layers through parallax depth (drift + sink + tilt scaled per layer) while content cards glide through 3D space — tilting toward you, sinking and settling as they cross the viewport — with a slim gold **depth rail** (desktop) tracking scroll progress live. Engineered for safety: per-element `perspective()` transforms only (ancestors untouched, so fixed overlays/nav/dock are unaffected), transform-only rAF-throttled updates, **small-layers-only rule** (giant `.hero`/`#step-class` containers stay flat after headless-compositor testing proved huge 3D layers unsafe; elements over 800px are skipped), and a full off-switch honoring both the in-app 🐢 reduced-motion toggle and `prefers-reduced-motion` (transforms cleared, rail hidden, state restored on re-enable). Zero-jump preserved — the engine never scrolls or focuses. Verified: v40 suite 10/10 (source + real-Chrome 375/1280), bug-hunt smoke tour ALL CLEAN with 3D active, v14 + v20–v39 regressions green, 12-viewport audit 0 issues, boot wire 264,917 B ≤ 266,240 B (**unchanged** — the whole feature ships lazy), bank.js untouched, SW `-v40`. |
|  Apple website look | **v41.0** — the whole site now looks like an **Apple website**: SF system type, Apple neutrals (`#fbfbfd`/`#f5f5f7`/`#1d1d1f`), a pure-black **keynote hero** with a blue→violet→pink marketing-gradient headline, a frosted-glass **global nav** (Study · Practice · AI Coach · Study Hall · HQ · Arcade · Notes · Teaching · Sign in), `#0071e3` **pill buttons** (980px), 18px cards, an Apple-ID-style white **sign-in card**, an Apple footer, and a full **dark-mode** set (true-black + `#2997ff`). Everything ships lazy in `quiz/polish.js` (`APPLECSS` + `appleTheme` + `appleNav`, scoped `html.apple.apple`) — **`index.html` byte-identical, wire 264,917 B unchanged**. Coexists with every prior feature: Theme Studio accents still apply (Apple accent pins yield via `:not(.st-acc-*)`, components follow `var(--green)`), card sheen/gradient, badge ring, pfBob/pfGlow/pfRingSpin animations, stat icons, ai-go shine, and the v40 3D engine all preserved (hidden stage layers use `visibility:hidden` so transforms still measure). Also fixed two visible pre-existing bugs: invalid `\uXXXX` CSS escapes rendering literally as `u2756` (setup heading) and `u2014 u2756 u2014` (footer ornament). Verified: v41 suite 12/12 (source + real-Chrome 375/1280 incl. signup→quiz smoke, answer states, dark-mode token flip), bug-hunt tour ALL CLEAN, v14 + v20–v40 regressions green (v24/v26 boot-safe regex and v40 SW pin widened for the legitimate Apple scope), 12-viewport audit 0 issues, bank.js untouched (`cec0489e203fd4c6`), SW `-v41`. |
|  Apple look, continued | **v42.0** — the Apple transformation goes deeper: **hero CTAs** (`Start practicing` pill + `Explore the Study Hall ›` link, the signature apple.com hero pattern), an authentic **promo strip** under the global nav (`Exam-Room Labs… Open the Study Hall ›`), a full **SF/type sweep** (every remaining Georgia rule — headings, quiz options, question text, reviews, reports, certificates, arcade/notes headings — now SF; brown/gold section eyebrows neutralized to Apple gray), **blue actives across every lazy surface** (arcade/notes/edu tabs + chips follow `var(--green)`, navy pills included), Teaching-Suite gold edges neutralized, lab report card whitened, plus Apple **micro-details**: blue text selection, slim gray scrollbar, tactile press-scale on buttons/options/chips, soft card hover lift. All still lazy in `quiz/polish.js` (`appleCtas` + `applePromo` reuse dock handlers, never scroll/focus). Verified: v42 suite 9/9 (source + real-Chrome 375/1280 incl. CTA navigation), tour ALL CLEAN, v14 + v20–v41 green (v41 SW pin widened to v41+), audit 0 issues, wire 264,917 B unchanged, bank untouched, SW `-v42`. |

| 🔎 Find + 🎬 Studio Pro + ✨ AI Reels | **v37.0** — three media-and-discovery upgrades, all lazy (shell + bank.js + boot wire untouched). **🔎 Find button** — a visible Find button now sits first in the top nav on every screen and opens the Ctrl+K search palette (Pro Tools are fetched on demand if they have not idle-loaded yet); the palette exposes `window.__palOpen` for it. **🎬 Video Studio Pro** — the Arcade studio gains video **search**, **SS1/SS2/SS3 class filters** beside the subject chips, 16:9 **thumbnail cards** (offline-safe), **✓ Watched tracking** with a “Watched X of Y” count, an **⏭ Up-next** button under the player, and an **✨ AI Reels banner** deep-linking to the new engine. **✨ AI Explainer Reels** (`quiz/reels.js`, idle-loaded) — auto-generated captioned video lessons for **all 27 subjects × SS1–SS3 (81 reels)**, scripted on-device from the curriculum: title scene → one scene per topic (summary + key terms) → quick-check quiz scene with mid-scene answer reveal → drill outro. The 16:9 canvas player has gold-ink animation, staggered captions, **voiceover** (device TTS, persisted toggle), play/pause/prev/next, clickable **script + scene dots**, 1×/1.5×/2× speed, **fullscreen**, **⏺ WebM recording + download**, recent-reels, surprise reel, keyboard shortcuts and screen-reader live captions — all reduced-motion aware. Two real engine bugs were caught by screenshots before ship (rAF-vs-wall-clock freeze, loop dying on scene advance) and both are now covered by auto-advance + loop-liveness assertions. Verified: v37 suite 12/12 (jsdom + real-Chrome 375/1280: 81/81 reels build with ≥3 scenes, poster + playback pixel-painted, Find → palette → results, studio search/filter/watch/up-next/seen, record + recent, zero errors, no overflow), v14 arcade + v20–v36 regressions green, 12-viewport audit 0 issues, boot wire 264,784 B ≤ 266,240 B (unchanged), bank.js untouched, SW `-v37`. |

| 🗺 Curriculum Atlas + AI Brain | **v36.0** — a discovery layer over the full 27-subject curriculum, one new lazy boot-safe module (`quiz/atlas.js`, idle-loaded by polish; shell + bank.js + boot wire untouched). **🗺 Curriculum Atlas** — every subject as a crest card with live question counts and **your accuracy** from real attempts (per-subject `sp` scores with `subj`+`pct` fallback); tap in for **topic cards** (curriculum summary, sample-question count, 🔁 Drill + 🧠 Ask-AI per topic), the **🗓 3-term scheme of work**, and a 📝 20-question subject paper; 🔎 search spans subjects, topics and keywords (`marketing` finds Marketing *and* the Agricultural Science topic that teaches it); 📤 Share uses the Web Share API with clipboard fallback; 🎲 surprise drill starts a random topic. Topic drills reuse the shell's own `drillTopic` (fresh visitors auto-get the richest class), falling back to a subject paper when a topic has no tagged questions in class. **🧠 AI curriculum brain** — the tutor now answers from the lazy `window.CURR` library (anchored name/keyword/subject scoring, summaries + a hidden-answer self-test + an Atlas pointer), so all 27 subjects — History, Marketing, Insurance… — get real answers instead of fallbacks. **🏷 One-shot tagging** — Atlas runs the shell's `tagAll()` once the lazy bank lands, tagging all 267 curriculum questions so drills, the mastery map and worksheets see them. **✨ Atlas Shine** — launcher chips lift + glow gold on hover, and the nav gains a shadow once you scroll (`pf-scrolled`). Verified: v36 suite 11/11 (jsdom + real-Chrome 375/1280: 27 subjects, 4,167 questions, 267 tagged / 0 untagged, grid → search → detail → real drill, AI `curriculum` src with fact/fallback preserved, palette hooks, nav shadow, zero errors, no overflow), v20–v35 regressions green, 12-viewport audit 0 issues, boot wire 264,784 B ≤ 266,240 B (unchanged), bank.js untouched, SW `-v36`. |

| 🇳🇬 Nigerian Curriculum + Holo 3D Lab v2 | **v35.0** — the question bank now spans **the full Nigerian senior-secondary curriculum**: 8 new subjects (**History, Technical Drawing, Marketing, Insurance, Visual Arts, Music, Physical Education, Home Economics**) join the existing 13 + 6 to make **27 subjects** and **4,167 questions**, all through a lazy `quiz/curriculum.js` (shell + bank.js + boot wire untouched). Every subject gets curated WAEC-style drill questions with worked explanations, `window.CURR` topic entries and a 3-term × 9-week `window.SYLL` scheme of work — so topic drills, the mastery map and the notes platform all see the new curriculum automatically. **Holo 3D Lab v2:** two new molecules (**Ethanol, Sodium chloride crystal** — a full rock-salt lattice with green Cl / purple Na), two new surfaces (Wind over Waves, Twin Peaks), **Earth's orbiting Moon** in the solar system, **tap-any-world planet facts** (Mercury → Neptune + the Moon, drag-safe tap detection), and a **💾 PNG export** button that saves any scene as an image. Better molecule framing (centroid + max-distance normalisation, so chains like ethanol fit perfectly). **Worldwide-ready shell:** external PWA manifest + PNG app icons + apple-touch-icon, canonical URL, JSON-LD structured data, `lang="en"`; hero reads 4,167 questions / 27 subjects. **Auto-scroll fixed:** the question palette jump, review-screen switch and most view switches no longer yank you to the top mid-paper; only class/subject/length/quiz setup and the intentional home-button scroll still do. Verified: v35 suite (jsdom + real-Chrome 375/1280: 27 subjects injected, 4,167 questions, no-invalid/no-duplicate sweep, History paper runs, scroll position preserved after a palette jump, new molecules/surfaces canvas-painted via pixel checks, Moon visible, tap-info + PNG export work, zero errors, no overflow), v33 suite updated for 10/8 counts and green, v20–v34 regressions green, 12-viewport audit 0 issues, boot wire 264,784 B ≤ 266,240 B, bank.js untouched (SHA `cec0489e…`), SW `-v35`. |

| 🧮 AI Tutor + Calculator | **v34.0** — two new lazy boot-safe modules (`quiz/ai.js` + `quiz/calc.js`, idle-loaded by polish; shell untouched, boot wire unchanged). **🧮 Scientific Calculator** — a full scientific machine: safe shunting-yard parser (no eval), DEG/RAD for sin/cos/tan, ln/log, √, x², xʸ, %, factorial, π/e/Ans, memory M+/M−/MR/MC, implicit multiplication (2π, 3(4+1)), tappable history, and full keyboard support (Enter =, Backspace, Del clears, Esc closes). **🧠 AI Study Tutor** — an offline knowledge engine of ~70 curated WAEC/NECO facts across every subject (each with an examiner tip), Formula-Vault lookup, an "explain the current question" mode (hints only — answers stay hidden), **live study insights** from your own attempts (weakest subjects & topics, forgetting-risk for anything untouched 7+ days, streak, and a generated revision plan), and a 🔑 **bring-your-own-AI-key** tab (Gemini or OpenAI, key stored device-only, graceful library fallback). All three zones are in the Ctrl+K palette. Verified: v34 suite 13/13 — real-Chrome 375/1280 (buttons → 230.5, sin(30)=0.5 in DEG, Ans & memory reuse, keyboard entry, library answers with badges, insights/empty-state, key tab, zero errors, no overflow). The suite caught three real bugs before ship: unary-minus crash (`2^-2` → NaN, fixed with right-associative unary precedence), single-letter keyword false-matches (`"g"` matching inside "gibberish", fixed with word-boundary matching), and missing MR/M− buttons. v20–v33 regressions green, 12-viewport audit 0 issues, boot wire 264,776 B ≤ 266,240 B (unchanged — lazy modules only), bank.js untouched, SW `-v34`. |
